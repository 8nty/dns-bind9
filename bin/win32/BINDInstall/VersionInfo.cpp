...
CVersionInfo::CVersionInfo(CString filename) {
	...
	DWORD handle;
	DWORD viSize = GetFileVersionInfoSize((LPTSTR)(LPCTSTR)filename,
					      &handle);
	m_versionInfo = NULL;
	...
	if (viSize == 0) {
		...
		if (!GetFileVersionInfo((LPTSTR)(LPCTSTR)filename, handle,
					viSize, m_versionInfo))
		{
			if (m_versionInfo) {
...
#ifdef NOTUSED
BOOL
CVersionInfo::CopyFileCheckVersion(CVersionInfo &originalFile) {
...
BOOL
CVersionInfo::CopyFileNoVersion(CVersionInfo &originalFile) {
...
CVersionInfo::QueryStringValue(CString value) {
	...
	if (m_versionInfo) {
		...
		if (n >= 0 && (size_t)n < sizeof(queryString)) {
			if (VerQueryValue(m_versionInfo, queryString, &viBlob,
					  &blobLen))
				return ((char *)viBlob);
		}
	}
